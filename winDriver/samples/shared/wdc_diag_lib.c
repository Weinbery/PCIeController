...
static void WDC_DIAG_DeviceResourcesPrint(const WD_CARD *pCard, DWORD hCard, FILE *fp)
{
    int resources;
    DWORD i;
    ...
    if (!pCard)
    {
        WDC_DIAG_ERR("WDC_DIAG_DeviceResourcesPrint: Error - NULL card pointer\n");
        return;
    }
    ...
    for (i = 0, resources = 0; i < pCard->dwItems; i++)
    {
        ...
        case ITEM_IO:
            resources ++;
            fprintf(fp, "    I/O range [BAR %ld]: base 0x%"KPRI"X, size 0x%lX\n",
...
void WDC_DIAG_PciDeviceInfoPrintFile(WD_PCI_SLOT *pPciSlot, FILE *fp, BOOL dump_cfg) 
{
    ...
    if (!pPciSlot)
    {
        WDC_DIAG_ERR("WDC_DIAG_PciDeviceInfoPrint: Error - NULL PCI slot pointer\n");
        return;
    }
...
/* Print location and resources information for all connected PCI devices to file*/
