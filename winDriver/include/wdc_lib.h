...
typedef struct {
    ...
    WD_PCI_ID   deviceId[WD_PCI_CARDS];   /* Vendor and device IDs of matching devices found */
    WD_PCI_SLOT deviceSlot[WD_PCI_CARDS]; /* Device location information for matching devices found */
} WDC_PCI_SCAN_RESULT;

/* PCMCIA scan results information struct */
typedef struct {
    DWORD          dwNumDevices;             /* Number of matching devices found */
    WD_PCMCIA_ID   deviceId[WD_PCMCIA_CARDS];   /* Manufacturer and device IDs of matching devices found */
    WD_PCMCIA_SLOT deviceSlot[WD_PCMCIA_CARDS]; /* Device location information for matching devices found */
} WDC_PCMCIA_SCAN_RESULT;

/* Driver open options */
/* Basic driver open flags */
#define WDC_DRV_OPEN_CHECK_VER 0x1 /* Compare source files WinDriver version with that of the running WinDriver kernel */
#define WDC_DRV_OPEN_REG_LIC   0x2 /* Register WinDriver license */
/* Convenient driver open options */
#define WDC_DRV_OPEN_BASIC     0x0 /* No option -> perform only the basic open driver tasks, which are 
                                      always performed by WDC_DriverOpen (mainly - open a handle to WinDriver) */
#define WDC_DRV_OPEN_KP WDC_DRV_OPEN_BASIC /* Kernel PlugIn driver open options <=> basic */
...
#define WDC_DBG_OUT_DBM     0x1  /* Send WDC debug messages to the Debug Monitor */
#define WDC_DBG_OUT_FILE    0x2  /* Send WDC debug messages to a debug file (default: stderr) [User-mode only] */

#define WDC_DBG_LEVEL_ERR   0x10 /* Display only error WDC debug messages */
#define WDC_DBG_LEVEL_TRACE 0x20 /* Display error and trace WDC debug messages */
...
typedef enum {
    WDC_ADDR_RW_DEFAULT = 0x0, /* Default: memory resource - direct access; autoincrement on block transfers */
    WDC_ADDR_RW_NO_AUTOINC = 0x4 /* Hold device address constant while reading/writing a block */
} WDC_ADDR_RW_OPTIONS;
...
#define WDC_ReadMem8(addr, off) *((volatile BYTE *)((unsigned long)(addr) + (off)))
#define WDC_ReadMem16(addr, off) dtoh16(*((volatile WORD *)((unsigned long)(addr) + (off))))
#define WDC_ReadMem32(addr, off) dtoh32(*((volatile UINT32 *)((unsigned long)(addr) + (off))))
#define WDC_ReadMem64(addr, off) dtoh64(*((volatile UINT64 *)((unsigned long)(addr) + (off))))

#define WDC_WriteMem8(addr, off, val) *((volatile BYTE * )((unsigned long)(addr) + (off))) = (val)
#define WDC_WriteMem16(addr, off, val) *((volatile WORD * )((unsigned long)(addr) + (off))) = dtoh16(val)
#define WDC_WriteMem32(addr, off, val) *((volatile UINT32 *)((unsigned long)(addr) + (off))) = dtoh32(val)
#define WDC_WriteMem64(addr, off, val) *((volatile UINT64 *)((unsigned long)(addr) + (off))) = dtoh64(val)

/* Read/write a device's address space (8/16/32/64 bits) */
DWORD DLLCALLCONV WDC_ReadAddr8(WDC_DEVICE_HANDLE hDev, DWORD dwAddrSpace, DWORD dwOffset, BYTE *val);
...
DWORD DLLCALLCONV WDC_WriteAddr8(WDC_DEVICE_HANDLE hDev, DWORD dwAddrSpace, DWORD dwOffset, BYTE val);
...
DWORD DLLCALLCONV WDC_ReadAddrBlock(WDC_DEVICE_HANDLE hDev, DWORD dwAddrSpace,
    DWORD dwOffset, DWORD dwBytes, PVOID pData, WDC_ADDR_MODE mode,
...
DWORD DLLCALLCONV WDC_PciReadCfgBySlot(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, PVOID pData, DWORD dwBytes);
DWORD DLLCALLCONV WDC_PciWriteCfgBySlot(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, PVOID pData, DWORD dwBytes);
...
/* Read/write 8/16/32/64 bits from the PCI configuration space.
   Identify device by its location. */
DWORD DLLCALLCONV WDC_PciReadCfgBySlot8(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, BYTE *val);
DWORD DLLCALLCONV WDC_PciReadCfgBySlot16(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, WORD *val);
DWORD DLLCALLCONV WDC_PciReadCfgBySlot32(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, UINT32 *val);
DWORD DLLCALLCONV WDC_PciReadCfgBySlot64(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, UINT64 *val);

DWORD DLLCALLCONV WDC_PciWriteCfgBySlot8(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, BYTE val);
DWORD DLLCALLCONV WDC_PciWriteCfgBySlot16(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, WORD val);
DWORD DLLCALLCONV WDC_PciWriteCfgBySlot32(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, UINT32 val);
DWORD DLLCALLCONV WDC_PciWriteCfgBySlot64(WD_PCI_SLOT *pPciSlot, DWORD dwOffset, UINT64 val);

/* Read/write 8/16/32/64 bits from the PCI configuration space.
   Identify device by handle */
DWORD DLLCALLCONV WDC_PciReadCfg8(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, BYTE *val);
DWORD DLLCALLCONV WDC_PciReadCfg16(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, WORD *val);
DWORD DLLCALLCONV WDC_PciReadCfg32(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, UINT32 *val);
DWORD DLLCALLCONV WDC_PciReadCfg64(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, UINT64 *val);

DWORD DLLCALLCONV WDC_PciWriteCfg8(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, BYTE val);
DWORD DLLCALLCONV WDC_PciWriteCfg16(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, WORD val);
DWORD DLLCALLCONV WDC_PciWriteCfg32(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, UINT32 val);
DWORD DLLCALLCONV WDC_PciWriteCfg64(WDC_DEVICE_HANDLE hDev, DWORD dwOffset, UINT64 val);
